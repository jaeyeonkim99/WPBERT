...
class CharBertEncoder(nn.Module):
    ...
    def forward(self, char_hidden_states, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.output_hidden_states:
                all_hidden_states = all_hidden_states + (hidden_states,)
                all_char_hidden_states = all_char_hidden_states + (char_hidden_states, )
        ...
        if self.output_hidden_states:
            outputs = outputs + (all_hidden_states,) + (all_char_hidden_states, )
        if self.output_attentions:
            outputs = outputs + (all_attentions,)
        return outputs  # last-layer hidden state, (all hidden states), (all attentions)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        #self.decoder = nn.Linear(in_features = config.hidden_size, out_features = config.vocab_size, bias=Falsep)

        #self.decoder = nn.Parameter(torch.zeros(config.vocab_size, config.hidden_size))
        #self.bias = nn.Parameter(torch.zeros(config.vocab_size))
        
        self.decoder = nn.Parameter(torch.empty(config.vocab_size, config.hidden_size*2))
        self.bias = nn.Parameter(torch.empty(config.vocab_size))
        init.kaiming_uniform(self.decoder, a=math.sqrt(5))
        fan_in, _ = init._calculate_fan_in_and_fan_out(self.decoder)
    ...
    def forward(self, hidden_states):
        ...
        mlm_scores = torch.matmul(hidden_states, self.decoder.transpose(0,1))+self.bias
